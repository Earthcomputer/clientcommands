...
public class WindowMixin {
    @ModifyExpressionValue(method = "calculateScale", at = @At(value = "CONSTANT", args = "intValue=320"))
    private int getSafeZoneWidth(int original) {
        Minecraft mc = Minecraft.getInstance();
        if (mc.screen instanceof IScreenSafeZone safeZone) {
            original = Math.max(original, safeZone.getSafeZoneWidth());
        }
        return original;
    }

    @ModifyExpressionValue(method = "calculateScale", at = @At(value = "CONSTANT", args = "intValue=240"))
